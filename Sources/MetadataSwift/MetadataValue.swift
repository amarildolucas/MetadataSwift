...
public enum MetadataValue: Sendable, Codable {
    ...
    public init(_ value: Any) {
        ...
        case let number as NSNumber:
            // NSNumber catches bridged Bool values as well as numeric payloads.
    ...
    /// Bridges an NSDictionary into Metadata while dropping non-String keys with a debug warning.
    private static func metadata(from dictionary: NSDictionary) -> Metadata {
        ...
        for (rawKey, value) in dictionary {
            guard let key = rawKey as? String else {
                #if DEBUG
                fputs("MetadataValue only supports String keys; dropping key: \(String(describing: rawKey))\n", stderr)
                #endif
                continue
            }
    ...
    /// Unwraps the metadata into the closest Foundation representation.
    ...
    /// Returns the underlying string when `self` is `.string`.
    ...
    /// Returns the underlying integer when `self` is `.int`.
    ...
    /// Returns a `Double` when `self` is `.double` or widens an `.int`.
    ...
    /// Returns the underlying Bool when `self` is `.bool`.
    ...
    /// Returns the nested array when `self` is `.array`.
    ...
    /// Returns the nested `Metadata` when `self` is `.object`.
