//
//  MetadataValueConversionTests.swift
//  MetadataSwift
//
...
struct MetadataValueConversionBehaviourTests {
    ...
    @Test("NSDictionary entries with non-string keys are ignored with a debug warning")
    func dictionaryDropsNonStringKeys() {
        let dictionary: NSDictionary = [NSNumber(value: 1): "value"]
        let metadataValue = MetadataValue(dictionary)
        guard case let .object(metadata) = metadataValue else {
            Issue.record("NSDictionary should still map to the .object case")
            return
        }
